...
def call_cflow(c_fname, cflow, numbered_nesting=True, preprocess=False,extra_args=[]):
    cflow_cmd = [cflow]
...
def parse_args():
    ...
    parser.add_argument('-m', '--multi-page', default=False, action='store_true',
                        help='produce hyperref links between function calls '
                              ...
                              + 'source file.')
    parser.add_argument('-E', '--extra', default=False, nargs='+',
                        help='pass extra options to cflow command line, ')
...
def main():
    ...
    for c_fname in c_fnames:
        cur_str = call_cflow(c_fname, cflow, numbered_nesting=True,preprocess=preproc, extra_args=extra )
        cflow_strs += [cur_str]
